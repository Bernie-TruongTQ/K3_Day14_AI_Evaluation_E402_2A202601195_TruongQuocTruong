...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            overlap_ratio = len(chunk_tokens & expected_tokens) / len(expected_tokens) if expected_tokens else 0.0
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"Question: {question}\nAnswer: {answer}\nRubric:\n{rubric_str}\nPlease return the scores in JSON format like: {{\"criterion_name\": score_value}}"
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) >= 2:
            ...
            for i, score_dict in enumerate(scores_batch):
                scores = list(score_dict.get("scores", {}).values())
                if not scores:
                    continue
                avg_item = sum(scores) / len(scores)
                if i % 2 == 0:
                    even_scores.append(avg_item)
                else:
                    odd_scores.append(avg_item)
            
            if even_scores and odd_scores and len(even_scores) == len(odd_scores):
                higher_count = sum(1 for e, o in zip(even_scores, odd_scores) if e > o)
                if higher_count / len(even_scores) > 0.7:
                    positional_bias = True

        if not positional_bias and len(scores_batch) > 1:
            first_scores = list(scores_batch[0].get("scores", {}).values())
            first_avg = sum(first_scores) / len(first_scores) if first_scores else 0.0
            
            other_scores = []
            for sd in scores_batch[1:]:
                other_scores.extend(list(sd.get("scores", {}).values()))
        ...
        all_scores = []
        for sd in scores_batch:
            for score in sd.get("scores", {}).values():
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_f = sum(r.faithfulness for r in new_results) / new_total if new_total > 0 else 0.0
        new_avg_r = sum(r.relevance for r in new_results) / new_total if new_total > 0 else 0.0
        new_avg_c = sum(r.completeness for r in new_results) / new_total if new_total > 0 else 0.0

        base_avg_f = sum(r.faithfulness for r in baseline_results) / baseline_total if baseline_total > 0 else 0.0
        base_avg_r = sum(r.relevance for r in baseline_results) / baseline_total if baseline_total > 0 else 0.0
        base_avg_c = sum(r.completeness for r in baseline_results) / baseline_total if baseline_total > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        for r in failures:
            if r.failure_type:
                categories[r.failure_type] = categories.get(r.failure_type, 0) + 1
        return categories
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures, 1):
            ...
            fix = suggestions[i - 1] if (i - 1) < len(suggestions) else "Review pipeline configurations"
            status = "Open"
            rows.append(f"| {fid} | {ftype} | {rc} | {fix} | {status} |")
        return header + "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        for cat, count in sorted_categories:
            ...
            if "hallucination" in cat_lower:
                suggestions.append("Implement hallucination checker to filter unsupported claims")
            elif "irrelevant" in cat_lower:
                suggestions.append("Refine system prompts and query templates to improve answer relevance")
            elif "incomplete" in cat_lower:
                suggestions.append("Add few-shot examples showing complete answers to improve completeness")
            elif "off_topic" in cat_lower:
                suggestions.append("Improve user intent detection and classification before querying RAG")

        fallbacks = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Implement a reranker (e.g., cross-encoder) to position key contexts higher",
            "Fine-tune the system prompt with student service specific guidelines and domain knowledge"
        ]
